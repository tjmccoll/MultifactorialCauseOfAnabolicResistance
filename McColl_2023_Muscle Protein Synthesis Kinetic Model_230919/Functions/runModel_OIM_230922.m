function [t, x, x_conc, expData_conc, expData_mass, CostParameters, interventionDuration, plasmaVolume, ...
    skeletalMuscleVolume] = runModel_OIM_230922(Protocol, ExpDataFile, SubjectData, InitialConditions, ...
...
if isequal(Protocol, 'Calibration, young adults') || isequal(Protocol ,'Valid., Glynn (2010) 1.85g') ||  ...
    ...
    t_bolus = [t; t2];
        ...
        % Fm,0
        x_bolus(:,42) = KValueVector_given(9)*x_bolus(:,8)./x_bolus(:,20) + ... % Akt(T)-mediated MPB
            KValueVector_given(69)*x_bolus(:,8)./x_bolus(:,26) + ... % mTORC1-mediated MPB
            KValueVector_given(11)*x_bolus(:,7) ; % KIC reamination
        % Net Balance
        x_bolus(:,43) = KValueVector_given(6)*x_bolus(:,4) ...
            - KValueVector_given(7)*x_bolus(:,6);
        % MPS
        x_bolus(:,44) = KValueVector_given(15)*x_bolus(:,6).*x_bolus(:,28); % MPS, r15
        % MPB
        x_bolus(:,45) = KValueVector_given(9)*x_bolus(:,8) ./ x_bolus(:,20) + ... % Akt(T)-mediated MPB
            KValueVector_given(69)*x_bolus(:,8) ./ x_bolus(:,26); % mTORC1-mediated MPB
...
elseif isequal(Protocol ,'Valid., Mitchell (2015) pulse') 
    [t,x] = ode23s(@(t,x) modelDerivative_OIM_230922(t, x, KValueVector_given, OptimizeParameters, ab, ...
    ...
    x0_2 = x(x_length, :);
    x0_2(1) = AminoAcidInput_mass;
    [t2,x2] = ode23s(@(t2,x2) modelDerivative_OIM_230922(t2, x2, KValueVector_given, OptimizeParameters, ab, ...
    ...
    [t3,x3] = ode23s(@(t3,x3) modelDerivative_OIM_230922(t3, x3, KValueVector_given, OptimizeParameters, ab, ...
    ...
    [t4,x4] = ode23s(@(t4,x4) modelDerivative_OIM_230922(t4, x4, KValueVector_given, OptimizeParameters, ab, ...
    ...
    [t5,x5] = ode23s(@(t5,x5) modelDerivative_OIM_230922(t5, x5, KValueVector_given, OptimizeParameters, ab, ...
    ...
    t_pulse = [t; t2; t3; t4; t5];
        ...
        % Fm,0
        x_pulse(:,42) = KValueVector_given(9)*x_pulse(:,8)./x_pulse(:,20) + ... % Akt(T)-mediated MPB
            KValueVector_given(69)*x_pulse(:,8)./x_pulse(:,26) + ... % mTORC1-mediated MPB
            KValueVector_given(11)*x_pulse(:,7) ; % KIC reamination
        % Net Balance
        x_pulse(:,43) = KValueVector_given(6)*x_pulse(:,4) ...
            - KValueVector_given(7)*x_pulse(:,6); % Net Balance
        % MPS
        x_pulse(:,44) = KValueVector_given(15)*x_pulse(:,6).*x_pulse(:,28); % MPS, r15
        % MPB
        x_pulse(:,45) = KValueVector_given(9)*x_pulse(:,8) ./ x_pulse(:,20) + ... % Akt(T)-mediated MPB
            KValueVector_given(69)*x_pulse(:,8) ./ x_pulse(:,26); % mTORC1-mediated MPB
    
        % parameter 46 sets as nan's to allow for cost calculation to work
        % (the x_pulse array needs to contain values for each cost parameter to be calculated)
        x_pulse(:,46) = nan;
...
elseif isequal(Protocol ,'Signaling knockdown')
    ...
    [t,x] = ode23s(@(t,x) modelDerivative_OIM_230922(t, x, KValueVector_given_KD, OptimizeParameters, ...
        ab, KValueVector_factor, EqDur, GlucInfRate, LeuInfRate), ...
        [0 :0.1: EqDur], x0_mass);
    ...
    [t2,x2] = ode23s(@(t2,x2) modelDerivative_OIM_230922(t2, x2, KValueVector_given_KD, OptimizeParameters, ...
