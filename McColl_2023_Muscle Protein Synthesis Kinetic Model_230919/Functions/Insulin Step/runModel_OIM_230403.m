function [t, x, x_conc, expData_conc, expData_mass, CostParameters, interventionDuration, plasmaVolume, ...
    skeletalMuscleVolume] = runModel_OIM_230403(Protocol, ExpDataFile, SubjectData, InitialConditions, ...
    kValues, EqDur, RunDur, GlucInfRate, LeuInfRate, kValues_KD, insulinStep, insulinStepDur)
...
% Insulin simulation as a step function
elseif isequal(Protocol, 'Insulin step') 
    interventionDuration = 180; % min
    AminoAcidInput = 0; % grams
    InsulinStep = insulinStep; %10e-9; % nM
    InsulinStepDur = insulinStepDur; %60; % min
    CostParameters = [];     
...
if isequal(Protocol ,'Signaling knockdown') || isequal(Protocol, 'Basal p70S6K Analysis') || ...
    isequal(Protocol, 'p70S6K reduced, mTORC1 kinase increased') || isequal(Protocol, 'Insulin step')
    expData_conc = nan;
    expData_mass = nan; 
else
...
if isequal(Protocol, 'Calibration, young adults') || isequal(Protocol ,'Valid., Glynn (2010) 1.85g') ||  ...
    ...
    t_bolus = [t; t2];
        ...
        x_bolus(:,42) = KValueVector_given(9)*x_bolus(:,8) ./ x_bolus(:,10) + KValueVector_given(11)*x_bolus(:,7); % Fm,0
...
elseif isequal(Protocol ,'Valid., Mitchell (2015) pulse') 
    ...
    t_pulse = [t; t2; t3; t4; t5];
        ...
        x_pulse(:,42) = KValueVector_given(9)*x_pulse(:,8) ./ x_pulse(:,10) + KValueVector_given(11)*x_pulse(:,7); % Fm,0
...
elseif isequal(Protocol ,'Insulin step')
    ...
    x0_2 = x(x_length, :);
    x0_2(1) = AminoAcidInput_mass;
    x0_2(3) = InsulinStep * plasmaVolume; % converting insulin step concentration to moles

    % Insulin step
    [t2,x2] = ode23s(@(t2,x2) modelDerivative_OIM_230403_insulinStep(t2, x2, KValueVector_given, OptimizeParameters, ...
        ab, KValueVector_factor, EqDur, InsulinStepDur, GlucInfRate, LeuInfRate), ...
        [EqDur :0.1: (EqDur+InsulinStepDur)], x0_2);

    % Run post insulin step
    x2_length = size(x2,1); %vertical length (rows)
    x0_3 = x2(x2_length, :);

    [t3,x3] = ode23s(@(t3,x3) modelDerivative_OIM_230403_insulinStep(t3, x3, KValueVector_given, OptimizeParameters, ...
        ab, KValueVector_factor, EqDur, InsulinStepDur, GlucInfRate, LeuInfRate), ...
        [(EqDur+InsulinStepDur) :0.1: (EqDur+RunDur)], x0_3);
