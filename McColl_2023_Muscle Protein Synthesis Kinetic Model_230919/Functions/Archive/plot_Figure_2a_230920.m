function [CostIndividual, CostTotal, mps_integral_grams] = plot_Figure_2a_230920(figNum, t, x_conc, x_mass, ExpData, ...
...
r9 = kValues(9)*x_mass(:,8)./x_mass(:,20); % F_m,0 (1); MPB (p-Akt(T) mediated)
r11 = kValues(11)*x_mass(:,7); % F_m,0 (2)
% F_m0 = r9+r11;
r69 = kValues(69)*x_mass(:,8) ./ x_mass(:,26);
F_m0 = r9+r11+r69; 
...
[Title, ~, ~, CostIndividual, CostTotal] = CostCalculation_230920(t, x_conc_update, [AktS473_pos, AktS473T308_pos], p70S6KT389_pos, ...
    EquilibriumDuration, CostParameters, ExpData, mps_integral_grams);
...
subplotTight(height_y, length_x, 9:12)
    plot (t,F_ma, '-', 'LineWidth',1.5, 'Color',[0 0 1]) % F_m,a
    grid on
        ax=gca;
        ax.FontSize = fontSize_units;
    xlim(x_lim)
    ...
    xtickangle(Angle)
hold on
plot (t,F_m0, '-', 'LineWidth',1.5, 'Color',[1 0 0]) % F_m,0
...
elseif any(CostIndividual(:,1)==F_m0_pos) % text for F_m,0 cost
    ...
    textOutput = sprintf('F_{m,0}: V(p) = %.2f', CostIndividual(textRow,2));
