function [t, x, x_conc, expData_conc, expData_mass, CostParameters, interventionDuration, plasmaVolume, ...
    skeletalMuscleVolume] = runModel_OIM_230920(Protocol, ExpDataFile, SubjectData, InitialConditions, ...
...
if isequal(Protocol, 'Calibration, young adults') || isequal(Protocol ,'Valid., Glynn (2010) 1.85g') ||  ...
    ...
    [t,x] = ode23s(@(t,x) modelDerivative_OIM_230920(t, x, KValueVector_given, OptimizeParameters, ...
    ...
    [t2,x2] = ode23s(@(t2,x2) modelDerivative_OIM_230920(t2, x2, KValueVector_given, OptimizeParameters, ...
        ab, KValueVector_factor, EqDur, GlucInfRate, LeuInfRate), ...
    ...
    t_bolus = [t; t2];
        ...
        x_bolus(:,42) = KValueVector_given(9)*x_bolus(:,8)./x_bolus(:,20) + KValueVector_given(69)*x_bolus(:,8)./x_bolus(:,26) + ...
            KValueVector_given(11)*x_bolus(:,7) ; % Fm,0; insulin-mediated MPB & insulin-independent MPB
...
%         x_bolus(:,45) = KValueVector_given(9)*x_bolus(:,8) ./ x_bolus(:,10); % MPB, r9 (p-IR concentration)
        x_bolus(:,45) = KValueVector_given(9)*x_bolus(:,8) ./ x_bolus(:,20) + ...
            KValueVector_given(69)*x_bolus(:,8) ./ x_bolus(:,26); % MPB: r9 (p-Akt(T) concentration) + r69 (mTORC1 mediated)
...
%         x_bolus(:,45) = KValueVector_given(9)*x_bolus(:,8) ./ x_bolus(:,10); % MPB, r9 (p-IR concentration)
        x_bolus(:,45) = KValueVector_given(9)*x_bolus(:,8) ./ x_bolus(:,20); % MPB, r9 (p-Akt(T) concentration)
