 function [] = regressionDiagnostics_NB(model, file_modelName, FilePath)
...
figure(2);
plot(model.Fitted, model.Variables.NB, ...
    'ko', 'MarkerSize',5);
grid on;
hold on;
...
ylabel('Observed Values');
    obsMin = min(model.Variables.NB); fitMin = min(model.Fitted);
    obsMax = max(model.Variables.NB); fitMax = max(model.Fitted);
ylim([min(obsMin, fitMin)*0.9, max(obsMax, fitMax)*1.01]);
xlim([min(obsMin, fitMin)*0.9, max(obsMax, fitMax)*1.01]);
% xlim([-0.7 0.7])
% ylim([-1.05, 1])
hold off;
