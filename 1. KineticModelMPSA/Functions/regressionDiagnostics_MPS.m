function [] = regressionDiagnostics_MPS(model, file_modelName, FilePath)
...
figure(2);
plot(model.Fitted, model.Variables.MPS, ...
    'ko', 'MarkerSize',5);
grid on;
hold on;
...
ylabel('Observed Values');
    obsMin = min(model.Variables.MPS); fitMin = min(model.Fitted);
    obsMax = max(model.Variables.MPS); fitMax = max(model.Fitted);
%     obsMin = min(model.Variables.MPS_transform); fitMin = min(model.Fitted);
%     obsMax = max(model.Variables.MPS_transform); fitMax = max(model.Fitted);
...
% if SaveFile == 'Y'
    ...
    figure1 = figure(1);
    fileName_complete = sprintf(append(file_modelName, '_ResidualsVsFitted_%s'), datestr(now, 'yy-mm-dd HH-MM-SS'));
    figfile1 = fullfile(FilePath, fileName_complete);
    saveas(figure1, figfile1, 'pdf');
    
    figure2 = figure(2);
    fileName_complete = sprintf(append(file_modelName, '_FittedVsObersved_%s'), datestr(now, 'yy-mm-dd HH-MM-SS'));
    figfile2 = fullfile(FilePath, fileName_complete);
    saveas(figure2, figfile2, 'pdf');

    figure3 = figure(3);
    fileName_complete = sprintf(append(file_modelName, '_ResidualsHistogram_%s'), datestr(now, 'yy-mm-dd HH-MM-SS'));
