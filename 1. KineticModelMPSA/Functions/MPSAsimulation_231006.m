function [t, x, x_conc] = MPSAsimulation_231006(subjectData, concentrationsIV, kValues, eqDur, simDur, glucInfRate, leuInfRate)
    ...
    % Equilibrium
    [t,x] = ode23s(@(t,x) modelDerivative_OIM_230922(t, x, kValues, [], ...
    ...
    [t2,x2] = ode23s(@(t2,x2) modelDerivative_OIM_230922(t2, x2, kValues, [], ...
        [], [], eqDur, glucInfRate, leuInfRate), ...
    ...
    t_bolus = [t; t2];
        ...
        % Fm,0
        x_bolus(:,42) = kValues(9)*x_bolus(:,8)./x_bolus(:,20) + ... % Akt(T)-mediated MPB
            kValues(69)*x_bolus(:,8)./x_bolus(:,26) + ... % mTORC1-mediated MPB
            kValues(11)*x_bolus(:,7) ; % KIC reamination
        % Net Balance
        x_bolus(:,43) = kValues(6)*x_bolus(:,4) ...
            - kValues(7)*x_bolus(:,6);
        % MPS
        x_bolus(:,44) = kValues(15)*x_bolus(:,6).*x_bolus(:,28); % MPS, r15
        % MPB
        x_bolus(:,45) = kValues(9)*x_bolus(:,8) ./ x_bolus(:,20) + ... % Akt(T)-mediated MPB
            kValues(69)*x_bolus(:,8) ./ x_bolus(:,26); % mTORC1-mediated MPB
