...
if (sim_type == "consensus") {
  ## combined therapies 
  # median values
  mps_therapy = data.frame(read.csv("3. TargetedApproach/Output Plots/25-10-04 17-13-28/4_Therapeutic strategies/Consensus/median/therapy_consensus_mpsMedian - 25-11-18 20-39-17.csv",header=F,sep=','))
  mpb_therapy = data.frame(read.csv("3. TargetedApproach/Output Plots/25-10-04 17-13-28/4_Therapeutic strategies/Consensus/median/therapy_consensus_mpbMedian - 25-11-18 20-39-17.csv",header=F,sep=','))
  nb_therapy = data.frame(read.csv("3. TargetedApproach/Output Plots/25-10-04 17-13-28/4_Therapeutic strategies/Consensus/median/therapy_consensus_nbMedian - 25-11-18 20-39-17.csv",header=F,sep=','))
  # SD values
  mps_sd_therapy = data.frame(read.csv("3. TargetedApproach/Output Plots/25-10-04 17-13-28/4_Therapeutic strategies/Consensus/median/therapy_consensus_mpsSD - 26-03-24 09-55-46.csv",header=F,sep=','))
  mpb_sd_therapy = data.frame(read.csv("3. TargetedApproach/Output Plots/25-10-04 17-13-28/4_Therapeutic strategies/Consensus/median/therapy_consensus_mpbSD - 26-03-24 09-55-46.csv",header=F,sep=','))
  nb_sd_therapy = data.frame(read.csv("3. TargetedApproach/Output Plots/25-10-04 17-13-28/4_Therapeutic strategies/Consensus/median/therapy_consensus_nbSD - 26-03-24 09-55-46.csv",header=F,sep=','))
  ...
  ## combined therapies 
  # median values
  mps_therapy = data.frame(read.csv("3. TargetedApproach/Output Plots/25-10-04 17-13-28/4_Therapeutic strategies/worstCase/median/therapy_worstCase_mpsMedian - 25-11-18 21-35-34.csv",header=F,sep=','))
  mpb_therapy = data.frame(read.csv("3. TargetedApproach/Output Plots/25-10-04 17-13-28/4_Therapeutic strategies/worstCase/median/therapy_worstCase_mpbMedian - 25-11-18 21-35-34.csv",header=F,sep=','))
  nb_therapy = data.frame(read.csv("3. TargetedApproach/Output Plots/25-10-04 17-13-28/4_Therapeutic strategies/worstCase/median/therapy_worstCase_nbMedian - 25-11-18 21-35-34.csv",header=F,sep=','))
   # SD values
  mps_sd_therapy = data.frame(read.csv("3. TargetedApproach/Output Plots/25-10-04 17-13-28/4_Therapeutic strategies/worstCase/median/therapy_worstCase_mpsSD - 26-03-24 19-06-20.csv",header=F,sep=','))
  mpb_sd_therapy = data.frame(read.csv("3. TargetedApproach/Output Plots/25-10-04 17-13-28/4_Therapeutic strategies/worstCase/median/therapy_worstCase_mpbSD - 26-03-24 19-06-20.csv",header=F,sep=','))
  nb_sd_therapy = data.frame(read.csv("3. TargetedApproach/Output Plots/25-10-04 17-13-28/4_Therapeutic strategies/worstCase/median/therapy_worstCase_nbSD - 26-03-24 19-06-20.csv",header=F,sep=','))
...
## Calculated pathological baseline median and SD values
```{r}
## MPS
# median
...
# sd 
mps_pathBaseline_sd <- apply(mps_pathBaseline,1,sd)
mps_pathBaseline_sd <- data.frame(baselinePath = mps_pathBaseline_sd[-1]) # remove healthy condition
mps_sd_therapy <- cbind(mps_pathBaseline_sd, mps_sd_therapy)
...
#sd
mpb_pathBaseline_sd <- apply(mpb_pathBaseline,1,sd)
mpb_pathBaseline_sd <- data.frame(baselinePath = mpb_pathBaseline_sd[-1]) # remove healthy condition
mpb_sd_therapy <- cbind(mpb_pathBaseline_sd, mpb_sd_therapy)
...
# sd
nb_pathBaseline_sd <- apply(nb_pathBaseline,1,sd)
nb_pathBaseline_sd <- data.frame(baselinePath = nb_pathBaseline_sd[-1]) # remove healthy condition
nb_sd_therapy <- cbind(nb_pathBaseline_sd, nb_sd_therapy)
```
...
therapyNames <- c("Therap0", "Therap1", "Therap2", "Therap3", "Therap4", "Therap5", "Therap6", "Therap7",
                  "Therap4_5", "Therap4_6", "Therap5_6", "Therap1_4_6", "Therap4_5_6", "Therap1_4_5_6")
therapyNames_detail <- c("Pathological\nbaseline", "1. Splanchnic\nextraction", "2. Blood flow", "3. Insulin\nresistance", "4. [mTOR]",
                    ...
                    "4+5", "4+6", "5+6", "1+4+6", "4+5+6", "1+4+5+6")

rownames(mps_therapy) <- simulationsNames # ** update row length + names
rownames(mps_sd_therapy) <- simulationsNames
rownames(mpb_therapy) <- simulationsNames
rownames(mpb_sd_therapy) <- simulationsNames 
rownames(nb_therapy) <- simulationsNames
rownames(nb_sd_therapy) <- simulationsNames 
colnames(mps_therapy) <- therapyNames
colnames(mps_sd_therapy) <- therapyNames
colnames(mpb_therapy) <- therapyNames
colnames(mpb_sd_therapy) <- therapyNames
colnames(nb_therapy) <- therapyNames
colnames(nb_sd_therapy) <- therapyNames
...
mps_sd_therapy_long <- mps_sd_therapy %>%
  rownames_to_column("SimID") %>%
  pivot_longer(cols = starts_with("Therap"), names_to = "TherapyID", values_to = "SD")
mps_therapy_long <- mps_therapy_long %>%
  left_join(mps_sd_therapy_long, by = c("SimID", "TherapyID"))

mpb_therapy_long <- mpb_therapy %>%
  rownames_to_column("SimID") %>%
  pivot_longer(cols = starts_with("Therap"), names_to = "TherapyID", values_to = "Value")
mpb_sd_therapy_long <- mpb_sd_therapy %>%
  rownames_to_column("SimID") %>%
  pivot_longer(cols = starts_with("Therap"), names_to = "TherapyID", values_to = "SD")
mpb_therapy_long <- mpb_therapy_long %>%
  left_join(mpb_sd_therapy_long, by = c("SimID", "TherapyID"))

nb_therapy_long <- nb_therapy %>%
  rownames_to_column("SimID") %>%
  pivot_longer(cols = starts_with("Therap"), names_to = "TherapyID", values_to = "Value")
nb_sd_therapy_long <- nb_sd_therapy %>%
  rownames_to_column("SimID") %>%
  pivot_longer(cols = starts_with("Therap"), names_to = "TherapyID", values_to = "SD")
nb_therapy_long <- nb_therapy_long %>%
  left_join(nb_sd_therapy_long, by = c("SimID", "TherapyID"))
...
border_rows = c(seq(2, 98, by=15), # MPS
                seq(128, 224, by=15), # MPB 
                seq(254, 350, by=15)) # NB 
...
# Create labels (median + SD)
# SD values with smaller font size
musMetab_combined <- musMetab_combined %>%
  mutate(label = case_when(
    simType == "MPB" ~ paste0(
      round(Value, 3),                # median in normal size
      "<br><span style='font-size:4pt;'>(",       # SD smaller in pt
      round(SD, 3),
      ")</span>"
    ),
    TRUE ~ paste0(
      round(Value, 2),
      "<br><span style='font-size:4pt;'>(",
      round(SD, 2),
      ")</span>"
    )
  ))
...
mpsTherapy_heatmap <- ggplot(mpsTherapy_data, aes(x = TherapyID, y = fct_rev(SimID), fill = Value)) +
  ...
  geom_richtext(
    ...
    size = 2.25,        # controls the median size
    lineheight = 0.9) +
...
if (sim_type == "consensus") {
  ggsave(file = paste0("R visualization/therapeuticSimulations/no_CompensatorySignaling/consensus/therapeuticSim_heatmap_consensus_MPS", ".pdf"),
         plot = mpsTherapy_heatmap, 
         width = w_Heatmap, height = h_heatmap, units = "in", device = "pdf")
} else if (sim_type == "worst-case"){
  ggsave(file = paste0("R visualization/therapeuticSimulations/no_CompensatorySignaling/worstCase/therapeuticSim_heatmap_worstCase_MPS", ".pdf"),
         plot = mpsTherapy_heatmap,
         width = w_Heatmap, height = h_heatmap, units = "in", device = "pdf")
}
```
...
if (sim_type == "consensus") {
  ggsave(file = paste0("R visualization/therapeuticSimulations/no_CompensatorySignaling/consensus/therapeuticSim_heatmap_consensus_MPB", ".pdf"),
         plot = mpbTherapy_heatmap, 
         width = w_Heatmap, height = h_heatmap, units = "in", device = "pdf")
} else if (sim_type == "worst-case"){
  ggsave(file = paste0("R visualization/therapeuticSimulations/no_CompensatorySignaling/worstCase/therapeuticSim_heatmap_worstCase_MPB", ".pdf"),
         plot = mpbTherapy_heatmap,
        width = w_Heatmap, height = h_heatmap, units = "in", device = "pdf")
}
```
...
if (sim_type == "consensus") {
  ggsave(file = paste0("R visualization/therapeuticSimulations/no_CompensatorySignaling/consensus/therapeuticSim_heatmap_consensus_NB", ".pdf"),
         plot = nbTherapy_heatmap, 
        width = w_Heatmap, height = h_heatmap, units = "in", device = "pdf")
} else if (sim_type == "worst-case"){
  ggsave(file = paste0("R visualization/therapeuticSimulations/no_CompensatorySignaling/worstCase/therapeuticSim_heatmap_worstCase_NB", ".pdf"),
         plot = nbTherapy_heatmap,
        width = w_Heatmap, height = h_heatmap, units = "in", device = "pdf")
}
```
...
w_comb = 17; h_comb = 23*.9

if (sim_type == "consensus") {
  ggsave(file = paste0("R visualization/therapeuticSimulations/no_CompensatorySignaling/consensus/therapeuticSim_heatmap_consensus_combined", ".pdf"),
         plot = therapySim_combined,
         width = w_comb, height = h_comb, units = "cm", device = "pdf")
} else if (sim_type == "worst-case"){
  ggsave(file = paste0("R visualization/therapeuticSimulations/no_CompensatorySignaling/worstCase/therapeuticSim_heatmap_worst_combined", ".pdf"),
         plot = therapySim_combined,
         width = w_comb, height = h_comb, units = "cm", device = "pdf")
}
```