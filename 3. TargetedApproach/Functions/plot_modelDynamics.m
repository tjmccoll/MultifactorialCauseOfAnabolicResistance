function [mps_integral_grams] = plot_modelDynamics(figNum, t, x_conc, x_mass, ...
    RunDuration, EquilibriumDuration, FilePath, FileName, kValues, interventionDuration)
...
%       plot_Figure_2a_230221(figNum, t, x_conc, x_mass, ExpData, 
...
%     area(t(t_endEq(1):t_endRun(1)), x_mass(t_endEq(1):t_endRun(1), MPS_pos), ...
%         'FaceColor',[0.9 0.9 1], 'EdgeColor','none') % colour overall w/ -painter option (Vector figure)


legend({'MPS', 'MPB', 'NB'}, 'Location','northwest', 'NumColumns',3)
...
rectangle('Position', [t(mps_max_t)-38, 0.45*mps_max-0.45e-5, 64*1.2, 0.9e-5*1.2], ...
    'FaceColor',[1 1 1 0.9], 'EdgeColor',[1 1 1])
...
% if SaveFile == 'Y'
    ...
    % Adjust figure size for proper export
    set(figure1, 'Units', 'centimeters');
    figWidth = 30 * expand_x;
    figHeight = 15 * expand_y;
    set(figure1, 'Position', [1, 1, figWidth, figHeight]);
    set(figure1, 'PaperUnits', 'centimeters');
    set(figure1, 'PaperSize', [figWidth, figHeight]);           % crucial for tight layout
    set(figure1, 'PaperPositionMode', 'manual');
    set(figure1, 'PaperPosition', [0 0 figWidth figHeight]);    % [left bottom width height]

    % Save as vector-based PDF
    figfile1 = fullfile(FilePath, FileName + ".pdf");
    print(figure1, figfile1, '-dpdf', '-opengl'); % '-painter' allows for vector image
% end
